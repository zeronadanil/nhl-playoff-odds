...
def format_section(title, teams, ranked=True):
    lines = [title]
    for i, team in enumerate(teams, start=1):
        lines.append(format_row(team, i if ranked else None))
    lines.append("")
    return "\n".join(lines)
...
def format_wildcard_view(standings):
    ...
    parts = [
        "NHL CURRENT STANDINGS - WILDCARD",
        "",
        format_section("ATLANTIC DIVISION", atlantic[:3]),
        format_section("METROPOLITAN DIVISION", metro[:3]),
        format_section("EASTERN CONFERENCE WILD CARD", east_wc),
    ]

    if east_hunt:
        parts.append(format_section("EASTERN CONFERENCE IN THE HUNT", east_hunt, ranked=False))

    parts.extend([
        format_section("CENTRAL DIVISION", central[:3]),
        format_section("PACIFIC DIVISION", pacific[:3]),
        format_section("WESTERN CONFERENCE WILD CARD", west_wc),
    ])

    if west_hunt:
        parts.append(format_section("WESTERN CONFERENCE IN THE HUNT", west_hunt, ranked=False))

    return "\n".join(parts)


def format_division_view(standings):
    parts = ["NHL CURRENT STANDINGS - DIVISION", ""]
    for division in ["Atlantic", "Metropolitan", "Central", "Pacific"]:
        teams = get_division_teams(standings, division)
        parts.append(format_section(f"{division.upper()} DIVISION", teams))
    return "\n".join(parts)


def format_conference_view(standings):
    east = get_conference_teams(standings, "Eastern")
    west = get_conference_teams(standings, "Western")

    return "\n".join([
        "NHL CURRENT STANDINGS - CONFERENCE",
        "",
        format_section("EASTERN CONFERENCE", east),
        format_section("WESTERN CONFERENCE", west),
    ])


def format_league_view(standings):
    all_teams = sort_teams(standings)
    return "\n".join([
        "NHL CURRENT STANDINGS - LEAGUE",
        "",
        format_section("NHL LEAGUE STANDINGS", all_teams),
    ])
...
def render_view(view):
    ...
    if view == "wildcard":
        return format_wildcard_view(standings)
    if view == "division":
        return format_division_view(standings)
    if view == "conference":
        return format_conference_view(standings)
    if view == "league":
        return format_league_view(standings)

    raise ValueError(f"Unknown standings view: {view}")
